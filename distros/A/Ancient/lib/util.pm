...
XS modules can register functions with util's export system by including
C<util_export.h>:

    #include "util_export.h"

=head2 util_register_export_xs
...
import the function via C<use util qw(name)>.

Example (see C<t/xs/util_export_test/> for full working code):
...
=head2 Perl API
    ...
    util::register_export($name, \&coderef)  # Register Perl coderef
    util::has_export($name)                   # Check if registered
