...
static XS(xs_has_export) {
    dXSARGS;
    if (items != 1)
        croak("Usage: util::has_export($name)");
 ...
 *   util_register_export_xs(aTHX_ "my_func", xs_my_func);
 */
void util_register_export_xs(pTHX_ const char *name, XSUBADDR_t xs_func) {
    if (!g_export_hash) {
        croak("util_register_export_xs: util module not yet loaded");
    }

    STRLEN name_len = strlen(name);
    if (hv_exists(g_export_hash, name, name_len)) {
        croak("util_register_export_xs: '%s' is already registered", name);
    }
