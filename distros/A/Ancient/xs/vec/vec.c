...
static SV* vec_wrap(pTHX_ Vec *v) {
    ...
    rv = newRV_noinc(sv);
    sv_bless(rv, gv_stashpv("vec", GV_ADD));
...
static Vec* vec_from_sv(pTHX_ SV *sv) {
    ...
    if (!sv_isobject(sv) || !sv_derived_from(sv, "vec")) {
...
static OP* pp_vec_get(pTHX) {
    ...
    POPs;
    if (idx < 0 || idx >= v->len) {
        croak("vec::get: index %ld out of bounds (len=%ld)", (long)idx, (long)v->len);
    }
    mPUSHn(v->data[idx]);
    RETURN;
}
...
static OP* pp_vec_dot(pTHX) {
    ...
    POPs;
    if (a->len != b->len) {
        croak("vec::dot: vectors must have same length");
    }
...
static OP* pp_vec_add(pTHX) {
    ...
    POPs;
    if (a->len != b->len) {
        croak("vec::add: vectors must have same length");
    }
...
static OP* pp_vec_sub(pTHX) {
    ...
    POPs;
    if (a->len != b->len) {
        croak("vec::sub: vectors must have same length");
    }
    c = vec_create(aTHX_ a->len);
    c->len = a->len;
...
static OP* pp_vec_mul(pTHX) {
    ...
    POPs;
    if (a->len != b->len) {
        croak("vec::mul: vectors must have same length");
    }
    c = vec_create(aTHX_ a->len);
    c->len = a->len;
...
static OP* pp_vec_div(pTHX) {
    ...
    POPs;
    if (a->len != b->len) {
        croak("vec::div: vectors must have same length");
    }
    c = vec_create(aTHX_ a->len);
    c->len = a->len;
...
static OP* pp_vec_add_inplace(pTHX) {
    ...
    if (a->len != b->len) {
        croak("vec::add_inplace: vectors must have same length");
    }
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::add_inplace: vector is read-only");
    }
...
static OP* pp_vec_scale_inplace(pTHX) {
    ...
    /* Don't pop - return self */
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::scale_inplace: vector is read-only");
    }
...
static OP* pp_vec_new(pTHX) {
    ...
    if (!SvROK(aref) || SvTYPE(SvRV(aref)) != SVt_PVAV) {
        croak("vec::new: argument must be an arrayref");
    }
...
static OP* pp_vec_set(pTHX) {
    ...
    if (idx < 0 || idx >= v->len) {
        croak("vec::set: index %ld out of bounds (len=%ld)", (long)idx, (long)v->len);
    }
    v->data[idx] = val;
    RETURN;
}
...
/* Helper: Check if an OP is accessing $_ (PL_defgv) 
 * Used to detect map/grep context where we need XS fallback
 */
static int vec_op_is_dollar_underscore(pTHX_ OP *op) {
    GV *gv;
    if (!op) return 0;
    
    /* Check for rv2sv -> gv pattern accessing $_ */
...
static OP* vec_unary_call_checker(pTHX_ OP *entersubop, GV *namegv, SV *ckobj, OP* (*pp_func)(pTHX)) {
    ...
    /* If self is $_, fall back to XS (map/grep context) */
    if (vec_op_is_dollar_underscore(aTHX_ selfop)) {
        return entersubop;
    }
...
static XS(xs_vec_zeros) {
    ...
    n = SvIV(ST(0));
    if (n < 0) croak("vec::zeros: size must be non-negative");
...
static XS(xs_vec_ones) {
    ...
    n = SvIV(ST(0));
    if (n < 0) croak("vec::ones: size must be non-negative");
...
static XS(xs_vec_fill) {
    ...
    n = SvIV(ST(0));
    val = SvNV(ST(1));
    if (n < 0) croak("vec::fill: size must be non-negative");
...
static XS(xs_vec_linspace) {
    ...
    n = SvIV(ST(2));
    if (n < 0) croak("vec::linspace: count must be non-negative");
...
static XS(xs_vec_set) {
    ...
    if (v->flags & VEC_FLAG_READONLY) {
        croak("vec::set: vector is read-only");
    }
...
static XS(xs_vec_add) {
    ...
    if (a->len != b->len) {
        croak("vec::add: vectors must have same length (%ld vs %ld)", 
...
static XS(xs_vec_eq) {
    ...
    if (a->len != b->len) {
        croak("vec::eq: vectors must have same length");
    }
...
static XS(xs_vec_ne) {
    ...
    if (a->len != b->len) {
        croak("vec::ne: vectors must have same length");
    }
...
static XS(xs_vec_lt) {
    ...
    if (a->len != b->len) {
        croak("vec::lt: vectors must have same length");
    }
...
static XS(xs_vec_le) {
    ...
    if (a->len != b->len) {
        croak("vec::le: vectors must have same length");
    }
...
static XS(xs_vec_gt) {
    ...
    if (a->len != b->len) {
        croak("vec::gt: vectors must have same length");
    }
...
static XS(xs_vec_ge) {
    ...
    if (a->len != b->len) {
        croak("vec::ge: vectors must have same length");
    }
...
static XS(xs_vec_where) {
    ...
    if (v->len != mask->len) {
        croak("vec::where: vectors must have same length");
    }
...
static XS(xs_vec_random) {
    ...
    n = SvIV(ST(0));
    if (n < 0) croak("vec::random: size must be non-negative");
...
static XS(xs_vec_add_scalar_inplace) {
    ...
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::add_scalar_inplace: vector is read-only");
    }
...
static XS(xs_vec_clamp_inplace) {
    ...
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::clamp_inplace: vector is read-only");
    }
...
static XS(xs_vec_sub_inplace) {
    ...
    if (a->len != b->len) {
        croak("vec::sub_inplace: vectors must have same length");
    }
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::sub_inplace: vector is read-only");
    }
...
static XS(xs_vec_mul_inplace) {
    ...
    if (a->len != b->len) {
        croak("vec::mul_inplace: vectors must have same length");
    }
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::mul_inplace: vector is read-only");
    }
...
static XS(xs_vec_div_inplace) {
    ...
    if (a->len != b->len) {
        croak("vec::div_inplace: vectors must have same length");
    }
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::div_inplace: vector is read-only");
    }
...
static XS(xs_vec_slice) {
    ...
    if (start < 0 || start >= a->len) {
        croak("vec::slice: start index out of bounds");
    }
    if (len < 0 || start + len > a->len) {
        croak("vec::slice: slice extends beyond vector");
    }
...
static XS(xs_vec_fill_range) {
    ...
    if (a->flags & VEC_FLAG_READONLY) {
        croak("vec::fill_range: vector is read-only");
    }
    if (start < 0) start = a->len + start;
    if (start < 0 || start >= a->len) {
        croak("vec::fill_range: start index out of bounds");
    }
    if (len < 0 || start + len > a->len) {
        croak("vec::fill_range: range extends beyond vector");
    }
...
static XS(xs_vec_fma_inplace) {
    ...
    Vec *c = vec_from_sv(aTHX_ ST(0));
    Vec *a = vec_from_sv(aTHX_ ST(1));
    Vec *b = vec_from_sv(aTHX_ ST(2));
    
    if (a->len != b->len || a->len != c->len) {
        croak("vec::fma_inplace: vectors must have same length");
    }
    if (c->flags & VEC_FLAG_READONLY) {
        croak("vec::fma_inplace: vector is read-only");
    }
...
static XS(xs_vec_axpy) {
    ...
    if (x->len != y->len) {
        croak("vec::axpy: vectors must have same length");
    }
    if (y->flags & VEC_FLAG_READONLY) {
        croak("vec::axpy: vector is read-only");
    }
...
static XS(xs_vec_std) {
    ...
    Vec *v = vec_from_sv(aTHX_ ST(0));
    if (v->len < 2) {
        ST(0) = sv_2mortal(newSVnv(0.0));
        XSRETURN(1);
    }
    
    double mean = vec_sum_impl(v->data, v->len) / v->len;
    double sum_sq = 0.0;
    IV i = 0;
...
static XS(xs_vec_dot) {
    ...
    double dot = vec_dot_impl(a->data, b->data, a->len);
...
static XS(xs_vec_norm) {
    ...
    Vec *v = vec_from_sv(aTHX_ ST(0));
    double dot = vec_dot_impl(v->data, v->data, v->len);
...
static XS(xs_vec_normalize) {
    ...
    Vec *a = vec_from_sv(aTHX_ ST(0));
    double norm = sqrt(vec_dot_impl(a->data, a->data, a->len));
    
    Vec *c = vec_create(aTHX_ a->len);
    c->len = a->len;
...
static XS(xs_vec_distance) {
    ...
    if (a->len != b->len) {
        croak("vec::distance: vectors must have same length");
    }
...
static XS(xs_vec_cosine_similarity) {
    ...
    if (a->len != b->len) {
        croak("vec::cosine_similarity: vectors must have same length");
    }
...
XS_EXTERNAL(boot_vec);
XS_EXTERNAL(boot_vec) {
    dXSARGS;
    PERL_UNUSED_VAR(items);
    
    /* Constructors */
    newXS("vec::new", xs_vec_new, __FILE__);
    newXS("vec::zeros", xs_vec_zeros, __FILE__);
    newXS("vec::ones", xs_vec_ones, __FILE__);
    newXS("vec::fill", xs_vec_fill, __FILE__);
    newXS("vec::range", xs_vec_range, __FILE__);
    newXS("vec::linspace", xs_vec_linspace, __FILE__);
    newXS("vec::random", xs_vec_random, __FILE__);
    
    /* Element access */
    newXS("vec::get", xs_vec_get, __FILE__);
    newXS("vec::set", xs_vec_set, __FILE__);
    newXS("vec::len", xs_vec_len, __FILE__);
    newXS("vec::to_array", xs_vec_to_array, __FILE__);
    
    /* Arithmetic */
    newXS("vec::add", xs_vec_add, __FILE__);
    newXS("vec::sub", xs_vec_sub, __FILE__);
    newXS("vec::mul", xs_vec_mul, __FILE__);
    newXS("vec::div", xs_vec_div, __FILE__);
    newXS("vec::scale", xs_vec_scale, __FILE__);
    newXS("vec::add_scalar", xs_vec_add_scalar, __FILE__);
    newXS("vec::neg", xs_vec_neg, __FILE__);
    newXS("vec::abs", xs_vec_abs, __FILE__);
    newXS("vec::sqrt", xs_vec_sqrt, __FILE__);
    newXS("vec::pow", xs_vec_pow, __FILE__);
    newXS("vec::exp", xs_vec_exp, __FILE__);
    newXS("vec::log", xs_vec_log, __FILE__);
    newXS("vec::sin", xs_vec_sin, __FILE__);
    newXS("vec::cos", xs_vec_cos, __FILE__);
    newXS("vec::tan", xs_vec_tan, __FILE__);
    newXS("vec::floor", xs_vec_floor, __FILE__);
    newXS("vec::ceil", xs_vec_ceil, __FILE__);
    newXS("vec::round", xs_vec_round, __FILE__);
    newXS("vec::asin", xs_vec_asin, __FILE__);
    newXS("vec::acos", xs_vec_acos, __FILE__);
    newXS("vec::atan", xs_vec_atan, __FILE__);
    newXS("vec::sinh", xs_vec_sinh, __FILE__);
    newXS("vec::cosh", xs_vec_cosh, __FILE__);
    newXS("vec::tanh", xs_vec_tanh, __FILE__);
    newXS("vec::log10", xs_vec_log10, __FILE__);
    newXS("vec::log2", xs_vec_log2, __FILE__);
    newXS("vec::sign", xs_vec_sign, __FILE__);
    newXS("vec::clip", xs_vec_clip, __FILE__);
    
    /* Comparison (return vec of 0/1) */
    newXS("vec::eq", xs_vec_eq, __FILE__);
    newXS("vec::ne", xs_vec_ne, __FILE__);
    newXS("vec::lt", xs_vec_lt, __FILE__);
    newXS("vec::le", xs_vec_le, __FILE__);
    newXS("vec::gt", xs_vec_gt, __FILE__);
    newXS("vec::ge", xs_vec_ge, __FILE__);
    
    /* Selection/Utility */
    newXS("vec::reverse", xs_vec_reverse, __FILE__);
    newXS("vec::concat", xs_vec_concat, __FILE__);
    newXS("vec::any", xs_vec_any, __FILE__);
    newXS("vec::all", xs_vec_all, __FILE__);
    newXS("vec::count", xs_vec_count, __FILE__);
    newXS("vec::where", xs_vec_where, __FILE__);
    newXS("vec::cumsum", xs_vec_cumsum, __FILE__);
    newXS("vec::cumprod", xs_vec_cumprod, __FILE__);
    newXS("vec::diff", xs_vec_diff, __FILE__);
    newXS("vec::sort", xs_vec_sort, __FILE__);
    newXS("vec::argsort", xs_vec_argsort, __FILE__);
    newXS("vec::isnan", xs_vec_isnan, __FILE__);
    newXS("vec::isinf", xs_vec_isinf, __FILE__);
    newXS("vec::isfinite", xs_vec_isfinite, __FILE__);
    
    /* In-place */
    newXS("vec::add_inplace", xs_vec_add_inplace, __FILE__);
    newXS("vec::sub_inplace", xs_vec_sub_inplace, __FILE__);
    newXS("vec::mul_inplace", xs_vec_mul_inplace, __FILE__);
    newXS("vec::div_inplace", xs_vec_div_inplace, __FILE__);
    newXS("vec::scale_inplace", xs_vec_scale_inplace, __FILE__);
    newXS("vec::add_scalar_inplace", xs_vec_add_scalar_inplace, __FILE__);
    newXS("vec::clamp_inplace", xs_vec_clamp_inplace, __FILE__);
    newXS("vec::fma_inplace", xs_vec_fma_inplace, __FILE__);
    newXS("vec::axpy", xs_vec_axpy, __FILE__);
    
    /* Slicing/Copy */
    newXS("vec::slice", xs_vec_slice, __FILE__);
    newXS("vec::copy", xs_vec_copy, __FILE__);
    newXS("vec::fill_range", xs_vec_fill_range, __FILE__);
    
    /* Reductions */
    newXS("vec::sum", xs_vec_sum, __FILE__);
    newXS("vec::product", xs_vec_product, __FILE__);
    newXS("vec::mean", xs_vec_mean, __FILE__);
    newXS("vec::variance", xs_vec_variance, __FILE__);
    newXS("vec::std", xs_vec_std, __FILE__);
    newXS("vec::median", xs_vec_median, __FILE__);
    newXS("vec::min", xs_vec_min, __FILE__);
    newXS("vec::max", xs_vec_max, __FILE__);
    newXS("vec::argmin", xs_vec_argmin, __FILE__);
    newXS("vec::argmax", xs_vec_argmax, __FILE__);
    newXS("vec::dot", xs_vec_dot, __FILE__);
    newXS("vec::norm", xs_vec_norm, __FILE__);
    
    /* Linear algebra */
    newXS("vec::normalize", xs_vec_normalize, __FILE__);
    newXS("vec::distance", xs_vec_distance, __FILE__);
    newXS("vec::cosine_similarity", xs_vec_cosine_similarity, __FILE__);
    
    /* Info */
    newXS("vec::simd_info", xs_vec_simd_info, __FILE__);
