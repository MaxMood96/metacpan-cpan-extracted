...
Switches to the fiber and passes C<@args>. Arguments can be of any Perl data type, including scalars, hash references,
...
tracked. Ideal for state machines or producer/consumer "dances". Like C<call>, it supports passing arbitrary Perl data
types via C<@args>.
...
C<try> occasionally leads to C<xcv_depth> errors and causes C<croak("Can't undef active subroutine");> crashes on exit
