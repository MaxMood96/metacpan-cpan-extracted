...
Affix supports C-style bitfields using a natural pipe (C<|>) syntax. This allows you to specify the number of bits for
a member.
...
Vectors (e.g. C<__m128>, C<__m256>, C<__m512> on x86; C<float32x4_t> on ARM) are first-class types in Affix. You can
...
=item * B<C<M256>>, B<C<M256d>>: 256-bit vectors (AVX)

=item * B<C<M512>>, B<C<M512d>>, B<C<M512i>>: 512-bit vectors (AVX-512)
...
Full examples can be found in the C<eg/> directory, Affix based modules are on CPAN (L<SDL3>) but here are a few to get
you started.

=head2 Working with Complex Types
...
=head2 Wrapping vs Affixing

Use C<affix> when you want to install a function directly into your current package:
...
Use C<wrap> when you want a scoped, anonymous code reference:
