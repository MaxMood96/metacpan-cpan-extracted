# Contributing to Alien::Xmake::Project
...
This builds `Alien::Xmake::Project` and installs a local xmake into `blib/`.

### Running the tests

    ./Build test

The suite exercises the real `xmake`/`xrepo` binaries, so it requires a working xmake (or the ability to fetch one). Tests build and install a small package (`zlib`) into a temporary, isolated store.
...
Open a [GitHub issue](https://github.com/sanko/Alien-Xmake-Project/issues) with:
...
- Repository: https://github.com/sanko/Alien-Xmake-Project
