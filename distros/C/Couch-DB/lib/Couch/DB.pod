...
The C<$version> can be a string or a version object (see "man version").
...
When a username is given, it will be used together with C<auth> and
...
provided via a L<Couch::DB::Database|Couch::DB::Database> object.  All methods which
relate to cluster management are implemented in L<Couch::DB::Cluster|Couch::DB::Cluster>,
...
Returns a L<Couch::DB::Node|Couch::DB::Node>-object with the C<$name>.  If the object does not
...
Returns a C<$count> number of UUIDs in a LIST.  This uses L<requestUUIDs()|Couch::DB/"Ungrouped calls">
...
(even C<undef>) which will free-up the memory use of the object immediately.
...
=item * C<all> =E<gt> BOOLEAN (default C<false>)
...
=head4 » Example: paging through result
...
=head4 » Example: paging with a website user session
...
=head4 » Example: get all results in a loop
...
=head4 » Example: get one page of results
...
=head4 » Example: get all results in one call
...
=head4 » Example: processing results when they arrive
...
=item Warning: $what got deprecated in api $release.

Cast by C<check()>

=item Error: $what got removed in $release, but you specified api $api.

Cast by C<check()>

=item Warning: $what was introduced in $release, but you specified api $api.

Cast by C<check()>

=item Error: No clients can run $method $path.
...
This module is part of Couch-DB version 0.201,
built on January 30, 2026. Website: F<http://perl.overmeer.net/CPAN/>
