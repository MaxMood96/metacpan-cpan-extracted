...
also hide changes in the server software, over time.  See L<Couch::DB::toPerl()|Couch::DB/"Processing">
and L<Couch::DB::toJSON()|Couch::DB/"Processing">
...
Returns the data for document revision C<$rev>, if retreived by a former
call.
...
Returns a HASH with information about one C<$revision> only.
...
As C<%options>: C<batch> and C<rev>.
...
document is in C<%data>.  When you pick your own document ids, you can also use
...
of C<%options> to collect additional meta-data.
