...
=item overload: B<bool> boolean context

These Return objects are overloaded to return a C<false> value when there is
...
=item couch => Couch::DB-object

=item on_chain => CODE|\@codes
...
Set the C<$code> and C<$message> to something else.  Your program should
...
Returns C<true> when this call may be chained with other calls to come up to
a number of rows which match the number you desire.  True when either
call parameter C<all> is C<true>, or a C<page_size> is given.

=item $obj-E<gt>B<isLastPage>()

Returns a C<true> value when there are no more page elements to be expected.  The
...
Returns a C<true> value when there should be made another attempt to fill the
...
The Result object is overloaded to produce a C<false> value when the command did
not succeed for any reason.

=head4 » Example: without error handler

  my $result = $db->find(...)
      or die $result->message;

=head4 » Example: with error handler
...
L<Couch::DB|Couch::DB> implementations will use event-driven programming to make this
...
=head4 » Example: prepare a query, delayed
...
To bridge the gap between your program and JSON data received, L<Couch::DB|Couch::DB>
...
=item Error: document not ready: $err
