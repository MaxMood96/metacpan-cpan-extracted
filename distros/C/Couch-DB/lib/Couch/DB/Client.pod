...
Return 'C<true>' if (this user logged-in to the server with) this client can perform
a certain role.
...
All C<%options> are posted as parameters.
...
Returns a C<true> value when the server status is "ok".
...
=item Error: Server info field does not contain the server version.

Cast by C<version()>

=item Error: Unsupport authorization '$how'

Cast by C<login()>

=item Error: did not get a node name for _local.
