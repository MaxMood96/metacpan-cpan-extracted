...
sub clone_or_create( $class, $orig, $new ) {

    !defined $new
      and return defined( $orig ) ? $orig->clone : undef;

    $new isa $class
      and return $new;

    if ( !defined $orig ) {
        return $class->can( 'assert_coerce' )
          ? $class->assert_coerce( $new )
          : assert_coerce_object( $class, $new );
    }

    my $attr;
    $class->can( 'coerce_attrs' ) && defined( $attr = $class->coerce_attrs( $new ) )
      and return $orig->clone( $attr->%* );

    is_plain_hashref( $new )
      and return $orig->clone( $new->%* );

    croak "cannot clone or create $class based upon input args";
}
...
sub assert_coerce_object ( $class, @iattrs ) {

    # pass through if the argument is already an object
    return $iattrs[0] if @iattrs == 1 && $iattrs[0] isa $class;

    load_module( $class );

    my $attr = $class->can( 'coerce_attrs' ) ? $class->coerce_attrs( @iattrs ) : undef;
    return $class->new( $attr->%* ) if defined $attr;

    return $class->new( @iattrs ) if @iattrs > 1;

    $attr = $iattrs[0];

    is_plain_hashref( $attr )
      and return $class->new( $attr->%* );

    is_ref( $attr )
      and croak( "$class: unable to extract attributes from non-hash reference" );

    croak( "$class: unable to extract attributes from non-reference scalar" );
}
...
sub pvalidate ( $name, $type, $value_ref ) {
    ...
    try {
        if ( $type isa Type::Tiny ) {
            ...
            load_module( $class );
            $value_ref->$*
              = $class->can( 'assert_coerce' )
              ? $class->assert_coerce( $value_ref->$* )
              : assert_coerce_object( $class, $value_ref->$* );
        }
    }
    catch ( $e ) {
...
my sub _colors {
    ...
    state @colors = do {
        ...
        map {
            ...
            CXC::Gnuplot::V0::Color->new( %color );
...
=head2 clone_or_create( $class, $orig, $new )

=head2 assert_coerce_object ( $class, @iattrs )
