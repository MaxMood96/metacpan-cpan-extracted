package CXC::Gnuplot::V1::CoordOffset2D;
...
class CXC::Gnuplot::V1::CoordOffset2D : isa(CXC::Gnuplot::V1::Base)
  does( CXC::Gnuplot::V1::Role::Clone );

use CXC::Gnuplot::V1::Types -lexical => 'CoordValue';
use Lexical::Import qw( Ref::Util is_plain_hashref );
...
method BUILDARGS : common (@args ) {
    return ( x => $args[0] ) if @args == 1 && !is_plain_hashref( $args[0] );
...
method opts {
    ...
    # remove trailing '0' offset coords with no coordsys
    $opts //= do {
        my @opts = map { $_->stringify } $x, $y;
        pop @opts while @opts && $opts[-1] eq '0';
        join( q{,}, @opts );
    };

    return length( $opts ) ? ( $opts ) : ();
}
...
reference. As shorthand, a single non-hash value is used as the C<x> parameter:
