...
The C<parse()> method returns C<undef>, if its string parameter (the first
parameter) cannot be parsed. The C<parse()> method croaks, if its string
parameter cannot be parse and if its options hash reference parameter (the
second parameter) has a C<fatal> boolean option that is true.
...
and the build extensions. The C<is_core()> predicate method indicates if a
version object has no pre-release extension and no build extension.
...
versions. The three-component dotted decimal version style is the newer one. It
matches the SemVer scheme but doesn't allow pre-release and build extensions.
...
As of version v2.2.0 this module (C<Version::Semantic>) contains
the cuckoo package C<Version::Core>. This package represents the
super-class implementation that the C<Version::Semantic> class inherits
from. C<Version::Core> provides the public methods C<corever_re()>,
C<parse()>, C<prefix()>, C<major()>, C<minor()>, C<patch()>, C<has_prefix()>,
C<version_core()>, C<compare_to()>, C<to_string()>, and C<increment()>
. Besides the constructor C<new()> the overridden methods are C<parse()>,
C<compare_to()>, C<to_string()>, and C<increment()>.

The C<corever_re()> class method provides the regular expression that is
used by the C<parse()> method. The C<parse()> method adds a C<\A> anchor at
the beginning and a C<\z> anchor at the end of the regular expression pattern.
