...
Each object has the required attributes C<major>, C<minor>,
and C<patch>.  Optional object attributes are C<pre_release> and
C<build>. C<has_pre_release()> and C<has_build()> predicate methods exist too.
...
strategies. Example:
  ...
  my $v_trial = Version::Semantic->parse( 'v1.0.0' )->increment( 'patch', 'TRIAL5' );
