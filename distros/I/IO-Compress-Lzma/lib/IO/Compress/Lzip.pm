...
use IO::Compress::Adapter::Lzip 2.224 ;
...
It returns an C<IO::Compress::Lzip> object on success and C<undef> on failure.
...
C<myfile.lz> and write some data to it.

    my $filename = "myfile.lz";
    my $z = IO::Compress::Lzip->new($filename)
        or die "IO::Compress::Lzip failed: $LzipError\n";
...
commandline, compresses it, and writes the compressed data to STDOUT.
    ...
    my $z = IO::Compress::Lzip->new("-", Stream => 1)
        or die "IO::Compress::Lzip failed: $LzipError\n";
...
To read the contents of the file C<file1.txt> and write the compressed
data to the file C<file1.txt.lz> there are a few options
...
Start by creating the compression object and opening the input file
    ...
    my $input = "file1.txt";
    my $z = IO::Compress::Lzip->new("file1.txt.lz")
        or die "IO::Compress::Lzip failed: $LzipError\n";
