...
It returns an C<IO::Compress::Lzma> object on success and C<undef> on failure.
...
C<myfile.lzma> and write some data to it.
...
To read the contents of the file C<file1.txt> and write the compressed
data to the file C<file1.txt.lzma> there are a few options
