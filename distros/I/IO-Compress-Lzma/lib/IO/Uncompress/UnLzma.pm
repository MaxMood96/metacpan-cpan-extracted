...
use IO::Uncompress::Adapter::UnLzma 2.224 ;
...
A top-level function, C<unlzma>, is provided to carry out
...
characters "<" and ">" C<unlzma> will assume that it is an
...
The optional parameters for the one-shot function C<unlzma>
...
C<unlzma> that are filehandles.

If C<AutoClose> is specified, and the value is true, it will result in all
input and/or output filehandles being closed once C<unlzma> has
completed.
...
To uncompress all files in the directory "/my/home" that match "*.txt.lzma" and store the compressed data in the same directory

    use strict ;
    use warnings ;
    use IO::Uncompress::UnLzma qw(unlzma $UnLzmaError) ;

    unlzma '</my/home/*.txt.lzma>' => '</my/home/#1.txt>'
        or die "unlzma failed: $UnLzmaError\n";
...
and if you want to compress each file one at a time, this will do the trick
    ...
    for my $input ( glob "/my/home/*.txt.lzma" )
    {
        my $output = $input;
        $output =~ s/.lzma// ;
        unlzma $input => $output
            or die "Error compressing '$input': $UnLzmaError\n";
    }
...
IO::Uncompress::UnLzma can be used exactly like an L<IO::File|IO::File> filehandle.
...
either the C<close> method is called or the IO::Uncompress::UnLzma object is
destroyed.
...
When reading the compressed input data, IO::Uncompress::UnLzma will read it in
...
the IO::Uncompress::UnLzma object is destroyed (either explicitly or by the
...
If the C<AutoClose> option has been enabled when the IO::Uncompress::UnLzma
...
No symbolic constants are required by IO::Uncompress::UnLzma at present.

=over 5

=item :all

Imports C<unlzma> and C<$UnLzmaError>.
Same as doing this
