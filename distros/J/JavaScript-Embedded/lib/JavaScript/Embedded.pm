...
sub new {
    ...
    if ($timeout){
        croak "timeout option must be a number" if !JavaScript::Embedded::Vm::duk_sv_is_number( $timeout );
    }

    if ( $max_memory ){
        croak "max_memory option must be a number" if !JavaScript::Embedded::Vm::duk_sv_is_number( $max_memory );
        croak "max_memory must be at least 256k (256 * 1024)" if $max_memory < 256 * 1024;
    }

    my $self  = bless {}, $class;

    my $duk   = $self->{duk} = JavaScript::Embedded::Vm->perl_duk_new( $max_memory, $timeout );
    ...
    $THIS = bless { duk => $duk, heapptr => 0 }, "JavaScript::Embedded::Object";
...
sub null                   { $JavaScript::Embedded::NULL::null; }
sub true                   { $JavaScript::Embedded::Bool::true; }
sub false                  { $JavaScript::Embedded::Bool::false }
sub JavaScript::Embedded::_ { $NOARGS }
sub this                   { $THIS }
...
package JavaScript::Embedded::Vm;
use strict;
use warnings;
...
BEGIN {
    ...
    sub _get_path { &JavaScript::Embedded::C::libPath::getPath }
...
sub to_perl_object {
    ...
    return JavaScript::Embedded::Util::jsObject(
        {
            duk     => $self,
            heapptr => $heapptr
        }
...
sub to_perl {
    ...
    if ( $type == JavaScript::Embedded::DUK_TYPE_UNDEFINED ) {
        $ret = undef;
    }

    elsif ( $type == JavaScript::Embedded::DUK_TYPE_STRING ) {
        $ret = $self->get_utf8_string($index);
    }

    elsif ( $type == JavaScript::Embedded::DUK_TYPE_NUMBER ) {
        $ret = $self->get_number($index);
    }

    elsif ( $type == JavaScript::Embedded::DUK_TYPE_BUFFER ) {
        my $ptr = $self->get_buffer_data( $index, my $sz );
        $ret = peek( $ptr, $sz );
    }

    elsif ( $type == JavaScript::Embedded::DUK_TYPE_OBJECT ) {
        ...
        $self->enum( $index, JavaScript::Embedded::DUK_ENUM_OWN_PROPERTIES_ONLY );
        ...
        while ( $self->next( -1, 1 ) ) {
            ...
            if ( $self->get_type(-1) == JavaScript::Embedded::DUK_TYPE_OBJECT ) {
    ...
    elsif ( $type == JavaScript::Embedded::DUK_TYPE_BOOLEAN ) {
        my $bool = $self->get_boolean($index);
        if ( $bool == 1 ) {
            $ret = JavaScript::Embedded::Bool::true();
        }
        else {
            $ret = JavaScript::Embedded::Bool::false();
        }
    }

    elsif ( $type == JavaScript::Embedded::DUK_TYPE_NULL ) {
        $ret = JavaScript::Embedded::NULL::null();
    }

    elsif ( $type == JavaScript::Embedded::DUK_TYPE_POINTER ) {
        my $p = $self->get_pointer($index);
        $ret = bless \$p, 'JavaScript::Embedded::Pointer';
    }
...
package JavaScript::Embedded::Bool;
{
    use warnings;
    use strict;
    our ( $true, $false );
    ...
    BEGIN {
        ...
        my $t = 1;
        my $f = 0;
        $true  = $use_boolean ? boolean::true() : bless \$t, 'JavaScript::Embedded::Bool';
        $false = $use_boolean ? boolean::false() : bless \$f, 'JavaScript::Embedded::Bool';
    }
...
package JavaScript::Embedded::NULL;
{
    ...
    BEGIN {
        my $n = '';
        $null = bless \$n, 'JavaScript::Embedded::NULL';
    }

    sub null { $null }
}

package JavaScript::Embedded::Object;
{
    use warnings;
    use strict;
    use Carp;
    ...
    sub AUTOLOAD {
        ...
        return JavaScript::Embedded::Util::autoload( $self, $method, $duk, $heapptr, @_ );
    }
...
package JavaScript::Embedded::Function;
{
    use strict;
    use warnings;
    use Data::Dumper;
...
package JavaScript::Embedded::Util;
{
    use strict;
    use warnings;
    use Data::Dumper;
    ...
    sub autoload {
        ...
        my $type = $duk->get_type(-1);
        if (   $type == JavaScript::Embedded::DUK_TYPE_OBJECT
            || $type == JavaScript::Embedded::DUK_TYPE_BUFFER )
        {
    ...
    sub jsFunction {
        ...
        return bless $sub, "JavaScript::Embedded::Function";
    }
    ...
    sub jsObject {
        ...
        if ( $duk->is_function(-1) ) {
            return JavaScript::Embedded::Util::jsFunction( 'anon', $duk, $heapptr, $constructor );
        }
...
JavaScript::Embedded - Perl interface to Duktape embeddable javascript engine
...
JavaScript::Embedded implements almost all duktape javascript engine api, the c code is just
...
initiate JavaScript::Embedded with options
...
return a C<JavaScript::Embedded::Object> this method will die if you try to get a property
that is not of type 'object'
    ...
    # $personObject is a blessed 'JavaScript::Embedded::Object' object
    # so you can call internal

    my $person = $personObject->new('Joe');
    print $person->name, "\n"; # Joe

For more on how you can use C<JavaScript::Embedded::Object> please see
...
As of version C<2.2.0> C<JavaScript::Embedded> integrated some of
...
This method will have effect only if you intiated with max_memory option
    ...
    my $js = JavaScript::Embedded->new( max_memory => 1024 * 256 );
...
you can translate duktape api to perl
    ...
    #and here is how we can implement it in JavaScript::Embedded
...
Besides duktape api, C<JavaScript::Embedded::Vm> implements the following methods
...
Get object at index and return it as 'JavaScript::Embedded::Object', this
...
C<JavaScript::Embedded> exports the following by default
...
JavaScript::Embedded has C<alert> and C<print> functions available as global
...
C<JavaScript::Embedded> vm methods is a direct low level calls to duktape c library, so stepping
...
=head2 JavaScript::Embedded::Object

C<JavaScript::Embedded::Object> use overload and AUTOLOAD internally, so there is no way to guess if
...
Author of this fork:
Rene Schickbauer C<< <cavac@cpan.org> >>
