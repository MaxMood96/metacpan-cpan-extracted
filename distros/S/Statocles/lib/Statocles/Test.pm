...
#pod =sub build_test_site
#pod
#pod     my $site = build_test_site( %site_args )
#pod
#pod Build a site for testing. The build and deploy will be set correctly to temporary
#pod directories. C<%site_args> will be given to the L<Statocles::Site|Statocles::Site>
#pod constructor.
#pod
#pod You must provide a C<theme> (probably using the one in C<t/share/theme>).
#pod
#pod =cut
...
sub build_test_site {
    ...
    $site_args{store} ||= Path::Tiny->tempdir;
    $site_args{build_store} ||= Path::Tiny->tempdir;
    $site_args{deploy} ||= Path::Tiny->tempdir;
    ...
    return Statocles::Site->new(
        title => 'Example Site',
        store => $site_args{store},
        build_store => $site_args{build_store},
        deploy => $site_args{deploy},
        log => $log,
        %site_args,
    );
}

#pod =sub build_test_site_apps
#pod
#pod     my ( $site, $build_dir, $deploy_dir ) = build_test_site_apps( $share_dir, %site_args );
#pod
#pod Build a site for testing, with some apps. Returns the site, the build dir, and the
#pod deploy dir.
#pod
#pod =cut
...
sub build_test_site_apps {
    ...
    $site_args{store} //= Path::Tiny->tempdir;
    $site_args{build_store} = $build_dir;
    $site_args{deploy} = $deploy_dir;
...
#pod =sub build_temp_site
#pod
#pod     my ( $tmpdir, $config_fn, $config ) = build_temp_site( $share_dir );
#pod
#pod Build a config file so we can test config loading and still use
#pod temporary directories
#pod
#pod =cut
