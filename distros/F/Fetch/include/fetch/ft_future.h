...
static int hmf_is_future(pTHX_ SV *sv) {
    return sv && SvROK(sv) && SvOBJECT(SvRV(sv))
        && sv_derived_from(sv, "Fetch::Future");
}
...
static const char *hmf_class_of(pTHX_ SV *self) {
    ...
    return stash ? HvNAME(stash) : "Fetch::Future";
}
...
static void hmf_await(pTHX_ SV *self) {
    ...
    {
        SV *aw = get_sv("Fetch::Future::AWAIT", 0);
        if (aw && SvOK(aw)) {
            dSP;
            ENTER; SAVETMPS; PUSHMARK(SP);
            XPUSHs(self);
    ...
    croak("Cannot await a pending Fetch::Future without an event loop");
}
