...
Boolean. Enable BEP 51 (Infohash Indexing). Defaults to C<1>.
...
Updates the local node ID. This is typically used during B<node ID rotation> when a new external IP address is
detected.
...
Returns the current external IP address string as detected by the DHT network. Returns C<undef> if no consensus has
been reached yet.
