...
my $validators = {
                'fixed_line' => '1[2-9]\\d{5}',
                'geographic' => '1[2-9]\\d{5}',
                'mobile' => '
          111\\d{6}|
          (?:
            2[12]|
...
my $timezones = {
    ...
    sub new {
      ...
      my $number = shift;
      $number =~ s/(^\+265|\D)//g;
      my $self = bless({ country_code => '265', number => $number, formatters => $formatters, validators => $validators, timezones => $timezones, }, $class);
