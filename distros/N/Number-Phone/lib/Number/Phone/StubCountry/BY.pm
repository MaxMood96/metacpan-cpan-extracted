...
my $timezones = {
    ...
    sub new {
      ...
      my $number = shift;
      $number =~ s/(^\+375|\D)//g;
      my $self = bless({ country_code => '375', number => $number, formatters => $formatters, validators => $validators, timezones => $timezones, areanames => \%areanames}, $class);
      return $self if ($self->is_valid());
      $number =~ s/^(?:0|80?)//;
      $self = bless({ country_code => '375', number => $number, formatters => $formatters, validators => $validators, areanames => \%areanames}, $class);
