...
my $timezones = {
    ...
    sub new {
      ...
      my $number = shift;
      $number =~ s/(^\+242|\D)//g;
      my $self = bless({ country_code => '242', number => $number, formatters => $formatters, validators => $validators, timezones => $timezones, areanames => \%areanames}, $class);
