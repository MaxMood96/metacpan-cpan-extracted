    ...
    { dist_root => 'does-not-exist' },
    {
        add_files => {
            path(qw(source dist.ini)) => simple_ini(
                ...
                ['ExecDir'],       #
                ['MetaConfig'],    #
                ['Test::CVE', { skip => "CVE.SKIP", filename => "t/zippity.t" } ]
            ),
            path(qw(source CVE.SKIP)) => <<'SKIP',
CVE-2011-4116
SKIP
...
my $file      = $build_dir->child(qw(t zippity.t));
ok( -e $file, $file . ' created' );
my $content = $file->slurp_utf8;

note $content;

like $content, qr/^\s+skip\s+=\>\s+"CVE\.SKIP\",$/m, "has skip option";
