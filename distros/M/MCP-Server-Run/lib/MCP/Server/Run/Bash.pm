package MCP::Server::Run::Bash;
our $VERSION = '0.001';
use Mojo::Base 'MCP::Server::Run', -signatures;
...
Concrete L<MCP::Server::Run> subclass that executes commands by invoking
...
Implements L<MCP::Server::Run/execute>. Runs C<$command> under C<bash -c>,
...
=item * L<MCP::Server::Run> - Base class defining the C<run> MCP tool
