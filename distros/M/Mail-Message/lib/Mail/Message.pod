...
before the message is constructed.  The C<$object> must be a sub-class
of L<Mail::Message::Body|Mail::Message::Body>.  See also L<body()|Mail::Message/"The body"> and L<storeBody()|Mail::Message/"Internals">.
...
=item labels => \@pairs|\%settings

Initial C<%settings> of the labels.  In case of L<Mail::Box::Message|Mail::Box::Message> type
objectss, this shall reflect the state the message is in inside the
folder.  For newly constructed L<Mail::Message|Mail::Message> objectss, this may be
anything you want, because L<coerce()|Mail::Message/"Internals"> will take care of the folder
specifics once the message is added to one.

=item messageId => $id

The C<$id> string on which this message can be recognized.  If none specified
...
=item $class-E<gt>B<read>($fh|$text|\$text|\@lines, %options)
...
=item $obj-E<gt>B<replyPrelude>( [STRING|$field|$address|\@addrs|\@fields] )

Inherited, see L<Mail::Message::Construct::Reply/"Constructing a message">

=item $any-E<gt>B<replySubject>(STRING)
...
[3.018] Return C<true> when the printed message has a trailing "\n".  In
...
Returns C<true> if the message is a part of another message.  This is
...
top message is a single message, 'C<undef>' is returned.  When it is a
...
C<false> when sending failed even after retries.

The optional C<$mailer> is a L<Mail::Transport::Send|Mail::Transport::Send> object.  When the
C<$mailer> is not specified, one will be created and kept as default for
the next messages as well.

The C<%options> are mailer specific, and a mixture of what is usable for
...
L<Mail::Transport::Send::send()|Mail::Transport::Send/"Sending mail">.  That object also provides a C<trySend()>
...
C<$fh>, with all surrounding information which is needed to put
...
name.  The C<$fieldname> is case insensitive.  The I<unfolded body> of the
field is returned, stripped from any attributes.
See L<Mail::Message::Field::body()|Mail::Message::Field/"Attributes">.
...
Return (optionally after setting) the C<$head> of this message.
...
With options, a new C<$body> is set for this message.  This is B<not>
...
The C<$body> must be an (sub-)class of L<Mail::Message::Body|Mail::Message::Body>.  In this case,
...
When C<$body> is C<undef>, the current message body will be dissected from
...
Short for C<< $msg->body->decoded >>  All C<%options> are passed-on.

=item $obj-E<gt>B<encode>(%options)

Encode the message to a certain format.  Read the details in the
dedicated manual page L<Mail::Message::Body::Encode|Mail::Message::Body::Encode>.  The C<%options> which
...
The option default to 'C<ALL>', which will return the message itself for
single-parts, the nested content of a message/rfc822 object, respectively
the parts of a multipart without recursion.  In case of 'C<RECURSE>', the
...
'C<ACTIVE>' and 'C<DELETED>' check for the deleted flag on messages and
message parts.  The C<$filter> is a code reference, which is called for
...
returns the same as L<isDeleted()|Mail::Message/"Flags">, which is preferred.  When a C<true>
...
Return the value of the C<$label>, optionally after setting some values.  In
case of setting values, you specify key-value C<PAIRS>.
...
that they will not all evaluate to C<true>, although most of them will.
...
Returns the message which is the original of this message, when this
was created by a L<clone()|Mail::Message/"Constructors"> operation.

=item $class-E<gt>B<coerce>($message, %options)

Coerce a C<$message> into a L<Mail::Message|Mail::Message>.  In some occasions, for instance
...
instance from a general L<Mail::Message|Mail::Message> into a L<Mail::Box::Mbox::Message|Mail::Box::Mbox::Message>,
no copy is needed.  However, to coerce a L<Mail::Internet|Mail::Internet> object into
a L<Mail::Message|Mail::Message>, a lot of copying and converting will take place.

Valid MESSAGEs which can be coerced into L<Mail::Message|Mail::Message> objects
are of type
...
C<true> or C<false>, dependent on the body type.

=item $obj-E<gt>B<readBody>( $parser, $head, [$bodytype] )

Read a body of a message.  The C<$parser> is the access to the folder's
file, and the C<$head> is already read.  Information from the C<$head> is used
to create expectations about the message's length, but also to determine
the mime-type and encodings of the body data.

The C<$bodytype> determines which kind of body will be made and defaults to
the value specified by L<new(body_type)|Mail::Message/"Constructors">.
C<$bodytype> may be the name of a body class, or a reference
to a routine which returns the body's class when passed the C<$head> as only
argument.

=item $obj-E<gt>B<readFromParser>( $parser, [$bodytype] )

Read one message from file.  The C<$parser> is opened on the file.  First
...
The optional C<$bodytype> may be a body class or a reference to a code
which returns a body-class based on the header.

=item $obj-E<gt>B<readHead>( $parser, [$class] )

Read a head into an object of the specified C<$class>.  The C<$class> defaults to
L<new(head_type)|Mail::Message/"Constructors">.  The C<$parser> is the access to the folder's file.
...
Take the message-id from the C<STRING>, or create one when the C<undef>
is specified.  If not C<STRING> nor C<undef> is given, the current header
...
Represent an integer C<$value> representing the size of file or memory,
(which can be large) into a short string using M and K (Megabytes
and Kilobytes).  Without C<$value>, the size of the message head is used.
...
=item Error: Cannot include forward source as $kind.
...
=item Error: cannot coerce a $type object into a $me object.

Cast by C<coerce()>

=item Error: cannot include reply source as $kind.
...
=item Error: cannot read message from a $source}.
...
=item Error: coercion starts with some object, not '$type'.
...
Cast by C<forwardAttach()>

=item Error: method forwardEncapsulate requires a preamble.

Cast by C<forwardEncapsulate()>

=item Error: method forwardNo requires a To.

Cast by C<forwardNo()>

=item Error: method forwardNo requires a body.
...
=item Error: please install Email::Simple.

Cast by C<coerce()>

=item Error: please install MIME::Entity.

Cast by C<coerce()>

=item Error: please install Mail::Internet.

Cast by C<coerce()>

=item Error: select parts via '$what'?

Cast by C<parts()>

=item Warning: skipped unknown key '$key' in build.
...
This module is part of Mail-Message version 4.04,
built on February 04, 2026. Website: F<http://perl.overmeer.net/CPAN/>
