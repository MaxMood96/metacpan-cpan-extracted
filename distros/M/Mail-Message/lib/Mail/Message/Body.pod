...
The information about encodings must be taken from the specified C<$body>,
...
Specify C<@lines> each terminated by a newline, or one C<$text> which may
...
field content, or a real C<$field> object.
...
field content, or a real C<$field>.
...
Read the data from the specified C<$file> name, file C<$handle>, or object of
type C<IO::Handle>.

=item filename => $file

[3.001] Overrule/set C<$file> name for content-disposition.
...
The C<$message> where this body belongs to.

=item mime_type => STRING|$field|$mime

The type of data which is added.  You may specify a content of a header
line as STRING, or a C<$field> object.  You may also specify a L<MIME::Type|MIME::Type>
...
return a new encoded body.  This C<$field> is case-insensitive and relates
...
Returns a C<true> or C<false> value, depending on whether the body of this
message has been read from file.  This can only C<false> for a
...
Only C<true> for a message body which contains exactly one sub-message:
...
optionally setting it to a new C<$message> first.  If C<undef> is passed,
...
The argument can be a C<STRING> (which is converted into a field), or a
fully prepared header C<$field>.
...
The optional C<STRING> or C<$field> enforces a new encoding to be set, without the
...
(in transport it will become a CRLF).  An empty body will return C<true>
...
Print the body to the specified C<$fh> (defaults to the selected handle).
an IO::File object or... any object with a C<print()> method will do.
Nothing useful is returned.

=item $obj-E<gt>B<printEscapedFrom>($fh)

Print the body to the specified C<$fh> but all lines which start
...
C<$head>.  The body was created from raw data without the required information,
...
Move the registration of the message to a new location over C<$distance>.  This
is called when the message is written to a new version of the same
folder-file.

=item $obj-E<gt>B<read>( $parser, $head, $bodytype, [$chars, [$lines]] )

Read the body with the C<$parser> from file. The implementation of this method
will differ between types of bodies.  The C<$bodytype> argument is a class name
...
The C<$chars> argument is the estimated number of bytes in the body, or
C<undef> when this is not known.  This data can sometimes be derived from
the header (the C<Content-Length> line) or file-size.

The second argument is the estimated number of C<$lines> of the body.  It is less
useful than the C<$chars> but may be of help determining whether the message
...
=item Error: message body: illegal datatype '$type' for data option.

Cast by C<new()>

=item Error: message body: illegal datatype '$type' for file option.
