...
=item From => $string|$address|\@addresses
...
=item To => $string|$address|\@addresses
...
If you specify a fully prepared C<$body> object, it will be used as forwarded
...
The C<%options> are the same as for C<forward()> except that C<body> is
