...
{	my ($self, $part) = @_;
	$part->isMultipart or return $part;
...
NO_PARTS
	...
	my $rebuild  = Mail::Message::Part->new(head => $part->head->clone, container => undef);
...
{	my ($self, $part, %args) = @_;
	...
	my $newbody = ref($body)->new(based_on => $body, parts => \@newparts);
