...
Addresses may be specified as string, or a L<Mail::Address|Mail::Address> object, or as
array of L<Mail::Address|Mail::Address> objects.

All C<%options> which are not listed below AND start with a capital, will
...
The C<@lines> which to be added after the quoted reply lines.  Create a
...
The C<@lines> which will be added before the quoted reply lines.  If nothing
...
quoted body of the message.  C<STRING> must comply to the RFC822 email
address specification, and is usually the content of a C<To> or C<From>
header line.  If a C<$field> is given, the field's body must be
compliant.  Without argument --or when the argument is C<undef>-- a
slightly different line is produced.

In case an ARRAY of C<@addrs> or C<@fields> is passed, the first one is taken.
