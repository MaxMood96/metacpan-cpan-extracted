...
=item $class-E<gt>B<new>()

Inherited, see L<Mail::Message::Field::Structured/"METHODS">
...
Add an C<$address> to the field.  The addresses are organized in groups.  If no
group is specified, the default group is taken to store the address in.  If
no C<$address> is specified, the option must be sufficient to create a
L<Mail::Message::Field::Address|Mail::Message::Field::Address> from.  See the C<%options> of
...
Add a group of addresses to this field.  A C<$group> can be specified, which
is a L<Mail::Message::Field::AddrGroup|Mail::Message::Field::AddrGroup> object, or one is created for you
using the C<%options>.  The group is returned.
...
Returns the group of addresses with the specified C<$name>, or C<undef>
if it does not exist.  If C<$name> is C<undef>, then the default groep
is returned.
...
Try to destilate address information from the C<STRING>.   Returned are
...
Try to get a valid domain representation from C<STRING>.  Returned are the
...
=item Warning: ignoring addressless phrase '$phrase'.
...
=item Warning: illegal part in address field $name: $part.
