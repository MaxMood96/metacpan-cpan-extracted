...
the L<Mail::Box|Mail::Box> suite: it does not use the constructor
...
Create a new header field object.  Specify the whole C<$line> (encoded
...
C<$name> and field C<$body>.

In case you specify a single C<$body> object, or an ARRAY of objects,
these are processed to become suitable to fill a field, usually by simple
strification.  When you specify one or more L<Mail::Address|Mail::Address> objects, these
are transformed into a string using their C<format> method.  You may also
add one L<Mail::Message::Field|Mail::Message::Field>, whose body is taken.  In case of an array,
the elements are joined into one string with a comma.

The C<$attributes> can be exactly one string which may contain multiple attributes
