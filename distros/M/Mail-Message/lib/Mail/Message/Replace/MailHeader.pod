...
The C<$arg> is an array with header lines.
...
Extends L<"Attributes" in Mail::Message::Head::Complete|Mail::Message::Head::Complete/"Attributes">.
...
the header for the specified C<$line>.  The C<$index> is ignored, the unfolded
body of the field is returned.
Improves base, see L<Mail::Message::Head::Complete/"Access to the header">
...
Delete the fields with the specified C<$tag>.  The deleted fields are
...
Improves base, see L<Mail::Message::Head/"Access to the header">
...
Replace the field named C<$tag>. from place C<$index> (by default the first) by
the C<$line>.  When C<$tag> is C<undef>, it will be extracted from the C<$line> first.
...
Refold all fields in the header, to C<$length> or whatever L<fold_length()|Mail::Message::Replace::MailHeader/"Simulating Mail::Header">
returns.

=item $obj-E<gt>B<fold_length>( [[$tag], $length] )

Returns the line wrap, optionally after setting it to C<$length>.  The
old value is returned.  The C<$tag> argument is ignored, because it is
...
Extract the fields from the C<ARRAY>, if specified, and then fold the fields.
...
an error (die), C<COERCE> - rename them as Mail-From and C<KEEP>
- keep them.
...
Read the header from the C<$file>.
Improves base, see L<Mail::Message::Head/"Internals">
...
Remove the folding for all instances of C<$tag>, or all fields at once.
...
=item Error: bad Mail-From choice: '$pick'.
