...
references in the ordered list are automagically C<undef>'d.
...
Remove the fields from the header which are exactly named 'C<STRING>' (case
...
=head4 » see examples/reduce.pl in distribution
