...
=item overload: B<""> stringifaction
...
=item $class-E<gt>B<build>( [PAIR|$field], ... )

Undefined values are interpreted as empty field values, and therefore skipped.
Improves base, see L<Mail::Message::Head/"Constructors">

=item $obj-E<gt>B<clone>( [@names|ARRAY|Regexps] )

Make a copy of the header, optionally limited only to the header lines
specified by C<@names>.  See L<grepNames()|Mail::Message::Head::Complete/"Access to the header"> on the way these fields can be
used.
...
Extends L<"Attributes" in Mail::Message::Head|Mail::Message::Head/"Attributes">.
...
Re-fold all fields from the header to contain at most C<$integer> number of
characters per line.
...
When a C<$field> object is specified (some L<Mail::Message::Field|Mail::Message::Field> instance), that
will be added.  Another possibility is to specify a raw header C<$line>, or a
header line nicely split-up in C<$name> and C<$body>, in which case the
field constructor is called for you.

C<$line> or C<$body> specifications which are terminated by a new-line are considered
...
Add a C<$resent_group> (a L<Mail::Message::Head::ResentGroup|Mail::Message::Head::ResentGroup> object) to
the header.  If you specify C<$data>, that is used to create such group
...
for details about the implementation of the C<$object>.
...
Count the number of fields with this C<$name>.  Most fields will return 1:
...
The C<@names> are considered regular expressions, and will all be matched
...
Print all headers to the specified C<$fh>, by default the selected
...
C<$fh>.  In this case, however, only the fields with names as specified by
C<STRING> (case insensative) or Regexp are printed.  They will stay the in-order
...
Like the usual L<print()|Mail::Message::Head::Complete/"Access to the header">, the header lines are printed to the specified
C<$fh>, by default the selected filehandle.  In this case, however,
...
Remove the specified C<$field> object from the header.  This is useful when
...
C<undef> and the field not printed.
...
Replace the values in the header fields named by C<$name> with the values
specified in the list of C<@fields>. A single name can correspond to multiple
...
If FIELDS is empty, the corresponding C<$name> fields will
...
some lines which contain spam fighting information.  When any C<$names> are
...
returned.  With more C<$names> or without C<$names>, a list will be returned
...
=item $any-E<gt>B<messageIdPrefix>( [$prefix, [$hostname]|CODE] )
...
providing a C<$prefix> and optionally a C<$hostname>, or a C<CODE> reference.

The C<CODE> reference will be called with the header as first argument.
You must ensure yourself that the returned value is RFC compliant.

The C<$prefix> defaults to C<mailbox-$$>, the C<$hostname> defaults to the
