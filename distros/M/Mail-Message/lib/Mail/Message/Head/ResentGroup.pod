...
is returned, taken from the specified C<$message> or message C<$head>.
...
C<@fields> are L<Mail::Message::Field|Mail::Message::Field> objects from the same header.

All C<%options> which start with capitals will be used to construct additional
...
=item Bcc => $text|$field|\@fields

=item Cc => $text|$field|\@fields
...
=item Delivered-To => $text|$field

=item From => $text|$field|\@fields

=item Message-ID => $text|$field

The C<Resent-Message-ID> which identifies this resent group.  The C<$field>
...
=item Return-Path => $text|$field

=item Sender => $text|$field
...
=item To => $text|$field|\@fields

=item head => $head

The C<$head> object where the data is stored in. By default a
L<Mail::Message::Head::Partial|Mail::Message::Head::Partial> is created for you.

=item message_head => $head

The real C<$head> of the message where this resent group is part of.  The
...
Improves base, see L<Mail::Message::Head::FieldGroup/"The header">
...
belongs to.  This may be C<undef> at creation, and then later filled in
...
Set a C<$field> to a (new) C<$value>.  The C<$field> names which do not start with
'Resent-*' but need it will have that added.  It is also an option to
specify a fully prepared message field C<$object>.  In any case, a field
C<$object> is returned.

» example: 

  my $rg = Mail::Message::Head::ResentGroup->new;
  $rg->set(To => 'fish@tux.aq');
  $msg->addResentGroup($rg);
  $msg->send;

  $msg->bounce(To => 'fish@tux.aq')->send;   # the same
  my $rg = Mail::Message::Head::ResentGroup->new(To => 'fish@tux.aq');
...
=item $any-E<gt>B<isResentGroupFieldName>($name)
