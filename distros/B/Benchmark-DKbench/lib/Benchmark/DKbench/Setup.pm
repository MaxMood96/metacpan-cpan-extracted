...
K/KA/KAMELKEV/CSS-Inliner-4027.tar.gz
C/CJ/CJFIELDS/BioPerl-1.7.8.tar.gz
E/ET/ETHER/Moose-2.4000.tar.gz
D/DA/DANKOGAI/Encode-3.21.tar.gz
D/DR/DROLSKY/DateTime-TimeZone-2.66.tar.gz
...
M/MI/MIK/CryptX-0.086.tar.gz
M/MI/MIK/Crypt-JWT-0.037.tar.gz
M/ML/MLEHMANN/JSON-XS-4.04.tar.gz
L/LE/LETO/Math-MatrixReal-2.13.tar.gz
R/RI/RIBASUSHI/SQL-Abstract-Classic-1.91.tar.gz
T/TO/TONYC/Imager-1.029.tar.gz
...
sub cpan_versions {
    ...
    my %versions;
    foreach (@packages) {
        m#/([a-z]+)(?:-([a-z]+))?(?:-([a-z]+))?(?:-([a-z]+))?-([0-9.]+).tar.gz$#i;
        warn $_ unless $1;
        my $mod = $1;
        $mod .= "::$2" if $2;
        $mod .= "::$3" if $3;
