...
# Global JWKS
ok( $res = $op->_get( '/oauth2/jwks', ), "Get default JWKS" );

my $keys = from_json( $res->[2]->[0] );
my @ids  = map { $_->{kid} } @{ $keys->{keys} };
is_deeply( \@ids, [ 'defaultkey', 'eckey' ], "Expected keys are returned" );
count(2);
...
my $keys = from_json( $res->[2]->[0] );
my @ids  = map { $_->{kid} } @{ $keys->{keys} };
is_deeply( \@ids, [ 'eckey', 'alt' ], "Expected keys are returned" );
count(2);
            ...
            ini => {
                ...
                oidcRPMetaDataOptions                 => {
                    rp => {
                        ...
                        oidcRPMetaDataOptionsIDTokenSignAlg        => "ES256",
                        oidcRPMetaDataOptionsAccessTokenSignAlg    => "ES256",
                        oidcRPMetaDataOptionsBypassConsent         => 0,
                        oidcRPMetaDataOptionsUserIDAttr            => "",
                        oidcRPMetaDataOptionsUserInfoSignAlg       => 'ES256',
                        ...
                        oidcRPMetaDataOptionsSigningKey   => 'eckey, alt',
                ...
                oidcServiceKeyIdSig      => 'defaultkey',
                oidcServiceSignatureKey  => 'default-oidc-sig, eckey',
                keys                     => {
                    "eckey" => {
                        keyPrivate => oidc_key_op_private_ec_sig(),
                        keyPublic  => oidc_key_op_public_ec_sig(),
                        keyId      => "eckey",
                    },
                    "alt" => {
...
"https://auth.op.com/.well-known/openid-configuration",
                        oidcOPMetaDataOptionsSigningKey              => 'eckey',
                        oidcOPMetaDataOptionsAuthnEndpointAuthMethod => 'jws',
                        oidcOPMetaDataOptionsAuthnEndpointAuthSigAlg => 'ES256',
                        oidcOPMetaDataOptionsTokenEndpointAuthMethod =>
                          'private_key_jwt',
                        oidcOPMetaDataOptionsTokenEndpointAuthSigAlg => 'ES256',
                    }
                },
                oidcServicePrivateKeySig => oidc_key_op_private_sig,
                oidcServicePublicKeySig  => oidc_cert_op_public_sig,
