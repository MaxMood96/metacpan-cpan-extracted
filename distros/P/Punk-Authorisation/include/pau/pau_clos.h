...
typedef struct { AV *cap; } pau_clos_t;

static int pau_clos_free(pTHX_ SV *sv, MAGIC *mg)
{
    pau_clos_t *c = (pau_clos_t *)mg->mg_ptr;
    PERL_UNUSED_ARG(sv);
    if (c) {
...
static MGVTBL pau_clos_vtbl = { NULL, NULL, NULL, NULL, pau_clos_free,
                                 NULL, NULL, NULL };

/* extra may be NULL; it is copied, so the caller keeps its own. */
static SV *pau_closure(pTHX_ XSUBADDR_t body, HV *cfg, SV *extra)
{
    ...
    Newxz(c, 1, pau_clos_t);
    c->cap = cap;
    sv_magicext((SV *)cv, NULL, PERL_MAGIC_ext, &pau_clos_vtbl, (char *)c, 0);
    return newRV_noinc((SV *)cv);
}

static AV *pau_cap_of(pTHX_ CV *cv)
{
    MAGIC *mg = mg_findext((SV *)cv, PERL_MAGIC_ext, &pau_clos_vtbl);
    AV *cap = mg ? ((pau_clos_t *)mg->mg_ptr)->cap : NULL;
    if (!cap) croak("Punk::Authorisation: a closure lost its capture");
    return cap;
}

static HV *pau_cfg_of(pTHX_ CV *cv)
{
    AV *cap = pau_cap_of(aTHX_ cv);
    SV **e = av_fetch(cap, 0, 0);
    if (!(e && *e && SvROK(*e) && SvTYPE(SvRV(*e)) == SVt_PVHV))
        croak("Punk::Authorisation: a closure lost its configuration");
    return (HV *)SvRV(*e);
}

/* The frozen argument, or NULL when the closure was made without one. */
static SV *pau_arg_of(pTHX_ CV *cv)
{
    AV *cap = pau_cap_of(aTHX_ cv);
...
static SV *pau_call_common(pTHX_ SV *inv, const char *meth, SV *code,
                            SV **argv, int argc)
{
    dSP;
    int count, i;
...
/* $inv->$meth(@argv) */
static SV *pau_call(pTHX_ SV *inv, const char *meth, SV **argv, int argc)
{
    return pau_call_common(aTHX_ inv, meth, NULL, argv, argc);
}
 ...
 * through here.
 */
static SV *pau_try(pTHX_ SV *inv, const char *meth, SV *code,
...
static SV *pau_await(pTHX_ SV *c, SV *v)
{
    SV *argv[2];
    if (!v) return newSV(0);
    ...
    argv[0] = c;
    argv[1] = v;
    return pau_call_common(aTHX_ NULL, "Punk::Auth::_await", NULL, argv, 2);
}

/* ---- predicates and small readers -------------------------------------- */

static int pau_is_hash(SV *sv)
{
    return sv && SvROK(sv) && SvTYPE(SvRV(sv)) == SVt_PVHV;
}

static int pau_is_array(SV *sv)
{
    return sv && SvROK(sv) && SvTYPE(SvRV(sv)) == SVt_PVAV;
}

/* One key of a hash, or NULL. Borrowed, not owned. */
static SV *pau_hget(pTHX_ HV *h, const char *k)
{
    SV **e = h ? hv_fetch(h, k, (I32)strlen(k), 0) : NULL;
    return (e && *e) ? *e : NULL;
}

static IV pau_hiv(pTHX_ HV *h, const char *k, IV dflt)
{
    SV *v = pau_hget(aTHX_ h, k);
...
enum { PAU_CX_ENV = 0, PAU_CX_APP = 1, PAU_CX_STASH = 4 };

static SV *pau_cx_slot(pTHX_ SV *c, I32 slot)
{
    AV *av;
    SV **e;
...
static HV *pau_app_of(pTHX_ SV *c)
{
    SV *app = pau_cx_slot(aTHX_ c, PAU_CX_APP);
    return pau_is_hash(app) ? (HV *)SvRV(app) : NULL;
}

/* Does the application declare a keyword? $c->session croaks without
 * `session` and $c->auth_id without `auth`, so the presence of the key is
 * asked before the method is called - an eval per request to find out what
 * boot already knew is not a bargain. */
static int pau_app_has(pTHX_ SV *c, const char *key)
{
    HV *app = pau_app_of(aTHX_ c);
    SV *v = app ? pau_hget(aTHX_ app, key) : NULL;
    return v && SvOK(v) ? 1 : 0;
}
...
static HV *pau_stash_of(pTHX_ SV *c)
{
    SV *s;
    SV *have = pau_cx_slot(aTHX_ c, PAU_CX_STASH);
    if (pau_is_hash(have)) return (HV *)SvRV(have);
    s = sv_2mortal(pau_call(aTHX_ c, "stash", NULL, 0));
    return pau_is_hash(s) ? (HV *)SvRV(s) : NULL;
}
