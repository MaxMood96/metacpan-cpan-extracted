...
static SV *pau_key_list(pTHX_ HV *h)
{
...
static SV *pau_name_list(pTHX_ const char *const *names)
{
    SV *out = sv_2mortal(newSVpvs(""));
    int i;
    for (i = 0; names[i]; i++) {
...
static void pau_check_opts(pTHX_ const char *who, const char *noun, HV *opts,
    ...
    while ((he = hv_iternext(opts))) {
        ...
        if (!ok)
            croak("%s: unknown %soption '%.*s' (known: %s)", who,
                  noun ? noun : "", (int)kl, k,
                  SvPV_nolen(pau_name_list(aTHX_ known)));
    }
}
...
static HV *pau_field_map(pTHX_ const char *who, const char *what,
        ...
        while ((he = hv_iternext(map))) {
            ...
            if (!hv_exists(out, k, (I32)kl)) {
                ...
                croak("%s: unknown %s '%.*s' (known: %s)", who, what,
                      (int)kl, k, SvPV_nolen(pau_name_list(aTHX_ names)));
            }
...
static void pau_helper(pTHX_ SV *app, const char *name, XSUBADDR_t body,
                        HV *cfg, SV *extra)
{
    SV *argv[2];
    argv[0] = sv_2mortal(newSVpv(name, 0));
    argv[1] = sv_2mortal(pau_closure(aTHX_ body, cfg, extra));
    SvREFCNT_dec(pau_call(aTHX_ app, "helper", argv, 2));
}

/* A keyword, owned by this plugin so a second install from the same owner is
 * the no-op Punk makes it. */
static void pau_keyword(pTHX_ SV *app, const char *name, XSUBADDR_t body,
                         HV *cfg, SV *extra, const char *owner)
{
    ...
    SvREFCNT_dec(pau_call(aTHX_ app, "install_kw", argv, 3));
}
 ...
 *
 * `pm` is the %INC key (Punk/Plugin/Feature.pm), `suffix` what to put in its
 * place (Plugin/Feature/sqitch). Mortal, or NULL when the module is not
 * in %INC - which happens when Punk skipped the require because the package
 * already had a register.
 */
static SV *pau_beside(pTHX_ const char *pm, const char *strip,
                       const char *suffix)
{
 ...
 */
static void pau_sqitch(pTHX_ SV *app, const char *name, SV *dir,
                        const char *const *engines)
{
    ...
    SvREFCNT_dec(pau_call(aTHX_ cls, "project", argv, 5));
}
 ...
 */
static void pau_ensure_model(pTHX_ SV *app, HV *cfg, const char *shipped)
{
    SV *name = pau_hget(aTHX_ cfg, "model");
    HV *apph;
    SV *models, *auto_sv;
    STRLEN nl;
    const char *np;
    ...
    models = pau_hget(aTHX_ apph, "models");
    if (pau_is_array(models)) {
    ...
    {   /* <App>::Model::<Name>, loadable? */
        SV *caller = sv_2mortal(pau_call(aTHX_ app, "caller_class", NULL, 0));
    ...
    auto_sv = pau_hget(aTHX_ apph, "model_auto");
    if (!named && !(auto_sv && SvOK(auto_sv))) {
        SV *one = sv_2mortal(newSViv(1));
        SvREFCNT_dec(pau_call(aTHX_ app, "model_auto", &one, 1));
    }
    {
        ...
        argv[0] = cls;
        SvREFCNT_dec(pau_call(aTHX_ app, "model_class", argv, 1));
...
static SV *pau_model_of(pTHX_ HV *cfg)
{
    SV *cls = pau_hget(aTHX_ cfg, "class");
    SV *app, *argv[1];

    if (!(cls && SvOK(cls))) croak("Punk::Authorisation: no application class");
    app = sv_2mortal(pau_call(aTHX_ cls, "punk_app", NULL, 0));
    if (!(app && SvROK(app)))
        croak("%s has not compiled yet; call this after to_app",
              SvPV_nolen(cls));
    argv[0] = pau_hget(aTHX_ cfg, "model");
    return pau_call(aTHX_ app, "model_instance", argv, 1);
}
...
static AV *pau_split(pTHX_ SV *sv)
{
    AV *out = newAV();
    STRLEN l, i, start = 0;
    const char *p;
 ...
 * column. Mortal. */
static SV *pau_join(pTHX_ AV *av)
{
    SV *out = sv_2mortal(newSVpvs(""));
...
static AV *pau_as_list(pTHX_ SV *sv)
{
    AV *out = newAV();
    if (!(sv && SvOK(sv))) return out;
    if (pau_is_array(sv)) {
...
static void pau_warn_once(pTHX_ SV *c, HV *cfg, const char *who, SV *msg)
{
    SV *w = pau_hget(aTHX_ cfg, "warned");
    HV *seen;
    STRLEN ml;
    const char *mp;
    SV *full;
    ...
    {
        SV *app = c ? sv_2mortal(pau_call(aTHX_ c, "app", NULL, 0)) : NULL;
        SV *log = (app && SvOK(app))
                ? sv_2mortal(pau_call(aTHX_ app, "log", NULL, 0)) : NULL;
        if (log && SvOK(log)) {
            SV *argv[1];
            argv[0] = full;
            SvREFCNT_dec(pau_call(aTHX_ log, "warn", argv, 1));
            return;
        }
    }
    warn("%s\n", SvPV_nolen(full));
