...
Kubernetes::REST::Apiregistration - DEPRECATED - v0 API group for API Registration resources
...
=head1 SYNOPSIS
    ...
    # Old way (deprecated):
    my $services = $api->Apiregistration->ListAPIService();

    # New way:
    my $services = $api->list('APIService');

=head1 DESCRIPTION

B<This module is DEPRECATED>. It provides backwards compatibility for the v0 API (Kubernetes::REST 0.01/0.02 by JLMARTIN) which used method names like C<< $api->Apiregistration->ListAPIService(...) >>.

The new v1 API uses simple methods directly on the main L<Kubernetes::REST> object:

    $api->list('APIService')
    $api->get('APIService', 'name')
    $api->create($apiservice)
